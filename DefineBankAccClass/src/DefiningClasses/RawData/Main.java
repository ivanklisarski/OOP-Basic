    ...
    public static void main(String[] args) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
            ...
            if ("fragile".equalsIgnoreCase(cargoType)) {
                cars.values().stream()
                        .filter(car -> "fragile".equalsIgnoreCase(car.getCargo().getCargoType()))
                        .filter(car -> car.getTires().stream().anyMatch(tire -> (tire.getTirePresure() < 1d)))
                        .forEachOrdered(car -> result.append(car.getModel()).append(System.lineSeparator()));
            } else {
                cars.values().stream()
                        .filter(car -> "flamable".equalsIgnoreCase(car.getCargo().getCargoType()))
